...
export default function MainLayout() {
  ...
  const sider = (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: 56, display: 'flex', alignItems: 'center', gap: 8, padding: '0 16px', color: '#fff', fontWeight: 700, fontSize: 15, whiteSpace: 'nowrap', overflow: 'hidden' }}>
        <CustomerServiceOutlined style={{ fontSize: 20, color: '#4f8cff' }} />
        {!collapsed && <span>企业增长云 · 电销台</span>}
      </div>
      <Menu
    ...
    <AntLayout style={{ minHeight: '100vh' }}>
      ...
      <AntLayout>
        <Header style={{ background: '#fff', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #f0f0f0' }}>
          ...
          <div style={{ flex: 1, color: '#888', fontSize: 13 }}>接线机器人 + 电销工作台 · 全链路演示</div>
          ...
          <Dropdown menu={{ items: [
            ...
            <span style={{ cursor: 'pointer' }}><Avatar size="small" style={{ background: '#4f8cff', marginRight: 6 }}>{user?.realName?.[0] || 'U'}</Avatar>{user?.realName}</span>
          </Dropdown>
        </Header>
        ...
        <Tabs items={[
          { key: 'notif', label: '站内通知', children: <List dataSource={notifs?.list || notifs || []} locale={{ emptyText: '暂无通知' }} renderItem={(n: any) => (
            <List.Item actions={[!n.isRead && <a key="r" onClick={async () => { await api(http.post(`/system/notifications/${n.id}/read`)); refetch(); }}>标为已读</a>]}>
              <List.Item.Meta title={n.title} description={<><div style={{ color: '#999', fontSize: 12 }}>{fmtTime(n.at)}</div><div>{n.content}</div></>} />
            </List.Item>
          )} /> },
          { key: 'help', label: '演示提示', children: <div style={{ color: '#666', lineHeight: 1.9 }}>
