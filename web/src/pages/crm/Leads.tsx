...
export default function Leads() {
  ...
  const { rows, total, isFetching } = usePaged(['leads'], '/leads', q);
  ...
  const create = async () => {
    ...
    message.success('线索已创建'); setOpen(false); form.resetFields(); qc.invalidateQueries({ queryKey: ['leads'] });
  };
  ...
  return (
    <Card title={<span className="page-title">线索池</span>} extra={
      <Space>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => setOpen(true)}>新建线索</Button>
      </Space>}>
        ...
        columns={[
          ...
          { title: '操作', render: (_, r) => (
            <Space onClick={(e) => e.stopPropagation()}>
              <Select size="small" style={{ width: 110 }} placeholder="分配" value={r.ownerId} onChange={(v) => assign(r.id, v)}
      ...
      <Modal title="新建线索" open={open} onOk={create} onCancel={() => setOpen(false)} destroyOnClose>
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="客户姓名"><Input /></Form.Item>
          <Form.Item name="company" label="公司名称"><Input /></Form.Item>
          <Form.Item name="phoneRaw" label="手机号" rules={[{ required: true, message: '请输入手机号' }]}><Input /></Form.Item>
          <Form.Item name="source" label="来源" initialValue="MANUAL">
            <Select options={SOURCES.map((v) => ({ value: v, label: lbl(v) }))} />
          </Form.Item>
          <Form.Item name="intentLevel" label="意向等级" initialValue="UNKNOWN">
